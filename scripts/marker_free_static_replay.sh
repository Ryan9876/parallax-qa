...
elif [ "${ALLOW_MARKER_FREE_README_ONLY:-0}" = "1" ]; then
  tmp_repo="$(mktemp -d)"
  trap 'rm -rf "${tmp_repo}"' EXIT
  git clone --quiet --depth=1 "${repo_url}" "${tmp_repo}/repo"
  source_paths="$(git -C "${tmp_repo}/repo" ls-tree -r --name-only HEAD)"
  if [ "${source_paths}" != "README.md" ]; then
    echo "Authorized QA target is not the exact README-only marker-free fixture: ${REPOSITORY_REF}" >&2
    printf '%s\n' "${source_paths}" >&2
    exit 1
  fi
  echo "QA phase: authorized README-only marker-free source verified (${REPOSITORY_REF})"
...
objective='This is an approved QA-only source-only marker-free greenfield acceptance test. Create a self-contained static web application that works offline with exactly the governed static-web source types: a root index.html, local styles.css, and local app.js. The page must visibly show the heading "Parallax QA Static", a count that starts at 0, and a button that increments the count. Keep all HTML, CSS, and JavaScript local. Do not create image, font, binary, SVG, or other asset files. Do not use frameworks, package managers, bundlers, repository scripts, external network resources, source publication, or application deployment. The complete source should be suitable for the protected static-web BUILD, TEST, and VERIFY contract.'
